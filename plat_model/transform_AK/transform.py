...
from .subgraph_extractors import *
...
class SubgraphsTransform(object):
    ...
    def __call__(self, adj, num_nodes):
        adj = adj.to(torch.long)
        subgraphs_nodes_mask, subgraphs_edges_mask, hop_indicator_dense = extract_subgraphs(adj,
                                                                                            num_nodes,
                                                                                            self.num_hops,
                                                                                            self.walk_length, self.p,
                                                                                            self.q, self.repeat)
        subgraphs_nodes, subgraphs_edges, hop_indicator = to_sparse(subgraphs_nodes_mask, subgraphs_edges_mask,
                                                                    hop_indicator_dense)
        subgraphs_nodes = subgraphs_nodes
        subgraphs_edges = subgraphs_edges
        hop_indicator = hop_indicator

        combined_subgraphs = combine_subgraphs(adj, subgraphs_nodes, subgraphs_edges,
                                               num_selected=num_nodes, num_nodes=num_nodes)

        subgraphs_batch = subgraphs_nodes[0]
        subgraphs_nodes_mapper = subgraphs_nodes[1]
        subgraphs_edges_mapper = subgraphs_edges[1]
        combined_subgraphs = combined_subgraphs
        hop_indicator = hop_indicator
        num_nodes = num_nodes  
        return subgraphs_batch, subgraphs_nodes_mapper, subgraphs_edges_mapper, combined_subgraphs, hop_indicator, num_nodes
...
def select_subgraphs(subgraphs_nodes, subgraphs_edges, hop_indicator, selected_subgraphs):
    ...
    batch_mapper[selected_subgraphs] = torch.arange(len(selected_subgraphs)).to(edges_batch.device)
...
def subsampling_subgraphs(edge_index, subgraphs_nodes, num_nodes=None, sampling_mode='shortest_path', random_init=False,
    ...
    if sampling_mode == 'random':
        selected_subgraphs, node_selected_times = random_sampling(subgraphs_nodes, num_nodes=num_nodes,
                                                                  rate=random_mode_sampling_rate,
                                                                  minimum_redundancy=minimum_redundancy)
    if sampling_mode == 'shortest_path':
        selected_subgraphs, node_selected_times = shortest_path_sampling(edge_index, subgraphs_nodes,
                                                                         num_nodes=num_nodes,
                                                                         minimum_redundancy=minimum_redundancy,
                                                                         stride=max(1, shortest_path_mode_stride),
                                                                         random_init=random_init)
    if sampling_mode in ['min_set_cover']:
        assert subgraphs_nodes.size(0) == num_nodes  
...
def combine_subgraphs(edge_index, subgraphs_nodes, subgraphs_edges, num_selected=None, num_nodes=None):
    ...
    node_label_mapper[subgraphs_nodes[0], subgraphs_nodes[1]] = torch.arange(len(subgraphs_nodes[1])).to(edge_index.device)
    node_label_mapper = node_label_mapper.reshape(-1)

    inc = (torch.arange(num_selected) * num_nodes).to(edge_index.device)
